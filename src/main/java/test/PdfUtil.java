...
public class PdfUtil {
            ...
            for (int i = 0; i < colorList.size() - 1; i++) {
                ...
                if (!current.endsWith(COLOR_TAG_END)) {
                    ...
                    if ((startTagIndex != -1) && (startTagIndex > endTagIndex)) {
                        ...
                        if (!colorList.get(i + 1).startsWith(COLOR_TAG_START)
                                && !colorList.get(i + 1).startsWith(COLOR_TAG_END)) {
    ...
    public static void drawBlueText(PDPageContentStream contentStream, String text, float x, float y, PDFont font,
                                    float fontSize) throws IOException {

        drawColorText(contentStream, text, x, y, font, fontSize, new Color(0, 128, 255));
    }

    public static void drawGrayText(PDPageContentStream contentStream, String text, float x, float y, PDFont font,
                                    float fontSize) throws IOException {

        drawColorText(contentStream, text, x, y, font, fontSize, new Color(142, 142, 142));
    }

    public static void drawColorText(PDPageContentStream contentStream, String text, float x, float y, PDFont font,
        ...
        if (text.indexOf(COLOR_TAG_START, start) != -1) {
            while (text.indexOf(COLOR_TAG_START, start) != -1) {
                ...
                contentStream.showText(text.substring(start, end)
                        .replace(COLOR_TAG_START, Constants.EMPTY)
                        .replace(COLOR_TAG_END, Constants.EMPTY));
    ...
    public static int getSubStringNextPositionByWidth(int start, float limitWidth, String text, PDFont font,
                                                      float fontSize) {

        AtomicInteger result = new AtomicInteger(start);
        AtomicReference<String> total = new AtomicReference<>(Constants.EMPTY);
        AtomicInteger count = new AtomicInteger(0);
        Arrays.stream(text.split(Constants.EMPTY)).skip(start).forEach(curChar -> {
            if (count.get() > 0) {
    ...
    public static List<Integer> calculateRowHeight(int startY, List<List<String>> rowData, PDFont font,
                                                   List<Integer> split) {
    ...
    public static float getStringWidth(String text, PDFont font, float fontSize) {
        try {
            text = text.replace(COLOR_TAG_START, Constants.EMPTY)
                    .replace(COLOR_TAG_END, Constants.EMPTY);
    ...
    public static void drawText(PDPageContentStream contentStream, String text, float x, float y, PDFont font,
                                float fontSize) throws IOException {
